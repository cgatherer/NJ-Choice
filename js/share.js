! function shareButton() {
      ...
      jQuery.fn.share = function(opts) {
            ...
            config.text = opts.text; // || jQuery('meta[name=title]').attr('content') || '';
            config.app_id = opts.app_id;
            config.title = opts.title;
            config.image = opts.image;
            config.button_color = opts.color || '';
            config.button_background = opts.background || '';
            config.button_icon = opts.icon || '';
            ...
            if (!jQuery('meta[name=sharer]').length) {
                  jQuery('head').append(getStyles(config));
            }
            jQuery(this).html("<label class='entypo-" + config.button_icon + "'><span>" + config.button_text + "</span></label><div class='social'><ul><li class='entypo-twitter' data-network='twitter'></li><li class='entypo-facebook' data-network='facebook'></li><li class='entypo-gplus' data-network='gplus'></li><li class='entypo-linkedin' data-network='linkedin'></li></ul></div>");
            paths = {
                  twitter: "http://twitter.com/intent/tweet?" + "text=" + config.fb_title + "&" + "url=" + config.twitter_url,
                  facebook: "https://www.facebook.com/sharer/sharer.php?u=" + config.fb_url,
                  gplus: "https://plus.google.com/share?url=" + config.gplus_url,
                  linkedin: "https://www.linkedin.com/shareArticle?mini=true&url=" + config.linkedin_url
            };
            ...
            click_link = function() {
                  ...
                  if ((jQuery(this).data('network') === 'facebook') && config.app_id) {
                        var sharelink = "https://www.facebook.com/dialog/feed?app_id=602752456409826&name="+config.fb_title+"&link="+config.fb_url+"&picture="+config.fb_image+"&caption="+config.fb_caption+"&description="+config.fb_text;
                        window.open(sharelink, 'targetWindow', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=500,height=350');
                  } else {

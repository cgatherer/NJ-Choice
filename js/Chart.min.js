...
(function() {
                  ...
                  initialize: function(t) {
                        ...
                        this.ScaleClass = i.Scale.extend({
                              ...
                              calculateBaseWidth: function() {
                                    return this.calculateX(1) - this.calculateX(0) - 2 * s.barValueSpacing - 30;
                              },
                  ...
                  buildScale: function(t) {
                        var i = this,
                              ...
                              n = {
                                    ...
                                    // xLabels: ["<h4>Label</h4><br><p>SubLabel</p>", "Okay"],
                                    xLabels: t,
      ...
      function() {
            ...
            i.Type.extend({
                  ...
                  // Princeton Parnters Changes Here

                        // get canvas

                        var dough = this.chart.canvas;
                        // draw circle to cover previous text
                        if (window.innerWidth > 700){
                              dough.getContext('2d').beginPath();
                              dough.getContext('2d').arc(dough.width/2, dough.width/2, (this.options.animateScale ? 0 : this.outerRadius / 100 * this.options.percentageInnerCutout), 0, 2 * Math.PI, false);
                              if (dough.parentNode.parentNode.classList.contains("whiteDoughnut")){
                                    dough.getContext("2d").fillStyle = "#fff";
                              } else {
                                    dough.getContext('2d').fillStyle = '#eeeeee';
                              }
                              dough.getContext('2d').fill();
                        }
                        var i = [],
                        ...
                        return e.each(this.segments, function(t) {
                              // draw text
                              if (window.innerWidth > 700){
                                    dough.getContext('2d').fillStyle = '#5F6062';
                                    dough.getContext('2d').font = "60px franklin_gothic_fscompressed";
                                    var w =  dough.getContext('2d').measureText(t.value + "%").width;
                                    t.inRange(s.x, s.y) && dough.getContext('2d').fillText(t.value + "%", (dough.width/2 - w/2), dough.width/2 - 10);
                                    dough.getContext('2d').font = "24px franklin_gothic_fscompressed";
                                    w =  dough.getContext('2d').measureText(t.label).width;
                                    if((w - 50) > (this.options.animateScale ? 0 : this.outerRadius / 100 * this.options.percentageInnerCutout)){
                                          dough.getContext('2d').font = "18px franklin_gothic_fscompressed";
                                          w =  dough.getContext('2d').measureText(t.label).width;
                                    }
                                    t.inRange(s.x, s.y) && dough.getContext('2d').fillText(t.label, (dough.width/2 - w/2), dough.width/2 + 20);
                              }else{
                              // old code

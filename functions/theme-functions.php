...
if( ! function_exists( 'mfn_add_image_size' ) )
{
	function mfn_add_image_size() {
		...
		// Blog & Portfolio -----------------------------------
			...
			add_image_size( 'portfolio-mf-t',   640,  1000, true  );	// Tall
	...
	function mfn_breadcrumbs( $class = false ){
		...
		// Single -----------------------------------------
			...
			// Blog | Single --------------------
				...
				$bread_title = explode(" ", get_the_title(),6);
				$breadcrumbs[] = '<a href="' . curPageURL() . '">'. $bread_title[0] . ' ' . $bread_title[1] . ' ' . $bread_title[2] . ' ' . $bread_title[3] .  ' ' . $bread_title[4] . '...</a>';
...
function mfn_search( $search_query ) {
	...
	if( is_search() && $search_query->is_main_query() && $search_query->is_search() ){
		...
		$post_ids = array_merge( $post_ids_post, $post_ids_meta );
